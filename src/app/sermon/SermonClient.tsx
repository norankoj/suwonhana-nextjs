...
const SERIES_TOPICS = [
  "고난주간", "부활절", "영적세계의 원리", "하나님의 속성",
  "예수님의 속성", "성령님의 속성", "BREAK THROUGH", "Destiny",
];
...
export default function SermonClient({
  ...
  // taxonomy는 서버에서 이미 로드됨 → 항상 true
  const [isLoading, setIsLoading] = useState(false);
  const [currentPage, setCurrentPage] = useState(1);
  const [totalPages, setTotalPages] = useState(initialTotalPages);

  // 필터가 초기 상태인지 확인 (초기 데이터 사용 가능 여부)
  ...
  // 필터 변경 시 → 페이지 1로 초기화 후 fetch
  useEffect(() => {
    // 초기 상태에서는 서버에서 받은 initialSermons 사용 (fetch 불필요)
    if (isDefaultFilter) {
  ...
  const fetchSermons = async (page = 1, signal?: AbortSignal) => {
    ...
    try {
      ...
      if (activeTab !== "전체") {
        ...
        else params.set("search", activeTab);
      }
      selectedTopics.forEach((topic) => {
        if (tagMap[topic]) tagIds.push(tagMap[topic]);
      });
      if (tagIds.length > 0)
        params.set("risen_multimedia_tag", tagIds.join(","));
      ...
      if (selectedBooks.length > 0) {
        ...
        if (uniqueIds) params.set("risen_multimedia_category", uniqueIds);
      }
      ...
      // 연도 필터
      if (selectedYear) {
        params.set("after", `${selectedYear}-01-01T00:00:00`);
        params.set("before", `${selectedYear}-12-31T23:59:59`);
      }
  ...
  const cleanContent = (html: string, hasVideo: boolean): string => {
    ...
    cleaned = cleaned
      ...
      .replace(
        /<(?:p|h[1-6]|div)[^>]*>(?:<(?:strong|b|em|span)[^>]*>)?\s*(?:말씀\s*)?(?:Audio|오디오)\s*(?:<\/(?:strong|b|em|span)>)?<\/(?:p|h[1-6]|div)>/gi,
        "",
      )
                      ...
                      <div className="flex flex-wrap items-center gap-4 py-4 border-t border-b border-slate-100 text-sm md:text-base text-slate-600">
                        ...
                        {audioUrl && (
                          <a
                            ...
                            className="ml-auto inline-flex items-center gap-2 px-4 py-2 bg-white hover:bg-slate-900 hover:text-white hover:border-slate-900 text-slate-700 border border-gray-300 rounded text-xs font-bold transition-colors shrink-0"
                          >
                            <Headphones size={14} />
                            MP3
