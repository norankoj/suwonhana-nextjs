...
export default function InquiryPage() {
  ...
  const isPasswordProtected = (post: InquiryPost) =>
    ...
    post.content.rendered.includes("password-protected");
            ...
            {viewMode === "list" && (
              <div>
                ...
                <div className="mt-12 border-t border-slate-100 pt-8 text-sm text-slate-500 leading-relaxed space-y-1.5">
                  ...
                  <p>
                    · 문의글은 비밀번호로 보호됩니다. 비밀번호를 꼭
                    기억해주세요.
                  </p>
                  <p>· 답변은 관리자가 확인 후 게시글에 달아드립니다.</p>
                  <p>
                    · 긴급 문의는 교회 사무실(031-202-0697)로 연락 바랍니다.
                  </p>
                </div>
              </div>
            )}
            ...
            {viewMode === "write" && (
              <div>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  {submitMsg && (
                    <div
                      className={`p-4 rounded text-sm font-medium ${submitMsg.includes("실패") ? "bg-red-50 text-red-600 border border-red-100" : "bg-slate-50 text-slate-700 border border-slate-200"}`}
                    >
                      {submitMsg}
                    </div>
                  )}
