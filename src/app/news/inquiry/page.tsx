        ...
        {viewMode === "list" && (
          <div>
            <div className="flex items-center justify-between mb-8">
              <h2 className="text-xl font-bold text-slate-900">문의 목록</h2>
              <button
            ...
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-24 text-center">
                ...
                <p className="text-slate-500 font-medium">아직 문의글이 없습니다.</p>
                <button
                  onClick={() => setViewMode("write")}
              ...
              <div className="divide-y divide-slate-100 border-t border-slate-100">
                {posts.map((post, i) => (
                  ...
                  >
                    <span className="text-slate-300 font-mono text-sm w-8 shrink-0 text-center">
                      {posts.length - i}
                    </span>
            ...
            <div className="mt-12 p-6 bg-slate-50 rounded-2xl border border-slate-100 text-sm text-slate-500 leading-relaxed">
              <p className="font-bold text-slate-700 mb-2">안내사항</p>
              <ul className="space-y-1 list-disc list-inside">
                <li>문의글은 비밀번호로 보호됩니다. 비밀번호를 꼭 기억해주세요.</li>
                <li>답변은 관리자가 확인 후 게시글에 달아드립니다.</li>
                <li>긴급 문의는 교회 사무실(031-203-3693)로 연락 바랍니다.</li>
              </ul>
            </div>
          </div>
        )}
        ...
        {viewMode === "write" && (
          <div>
            <button
              ...
              className="flex items-center gap-1 text-sm text-slate-500 hover:text-slate-900 mb-8 transition-colors font-bold"
            >
              <ChevronLeft size={16} /> 목록으로
            </button>
            <h2 className="text-2xl font-bold text-slate-900 mb-8">문의 작성</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                ...
                <input
                  ...
                  placeholder="작성자 이름"
                  className="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-slate-900 focus:ring-1 focus:ring-slate-900 transition-all"
                />
              </div>
              <div>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={8}
                  className="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-slate-900 focus:ring-1 focus:ring-slate-900 transition-all resize-none"
                />
              </div>
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full px-4 py-3 pr-12 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-slate-900 focus:ring-1 focus:ring-slate-900 transition-all"
                  />
                  <button
                    type="button"
              ...
              {submitMsg && (
                <div className={`p-4 rounded-xl text-sm font-medium ${submitMsg.includes("실패") ? "bg-red-50 text-red-600 border border-red-100" : "bg-emerald-50 text-emerald-600 border border-emerald-100"}`}>
                  {submitMsg}
                </div>
              )}
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-4 bg-slate-900 text-white rounded-xl font-bold text-sm hover:bg-slate-800 disabled:opacity-50 transition-colors"
              >
                {isSubmitting ? "등록 중..." : <><Send size={16} /> 문의 등록하기</>}
              </button>
            </form>
          </div>
        )}
          ...
          <div>
            ...
            {isPasswordProtected(selectedPost) ? (
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <h3 className="text-xl font-bold text-slate-900 mb-2">비밀 게시글입니다</h3>
                <p className="text-slate-500 text-sm mb-6">작성 시 설정한 비밀번호를 입력하세요.</p>
                <div className="w-full max-w-xs space-y-3">
                  <div className="relative">
                    <input
                      ...
                      placeholder="비밀번호"
                      className="w-full px-4 py-3 pr-12 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-slate-900 focus:ring-1 focus:ring-slate-900"
                    />
                    <button
                      type="button"
                      onClick={() => setShowPassword(!showPassword)}
              ...
              <div>
                ...
                <div
                  className="prose max-w-none text-slate-700 leading-loose text-base mb-10"
                ...
                {selectedPost.meta?.reply && (
                  <div className="mt-8 p-6 bg-blue-50 border border-blue-100 rounded-2xl">
                    <p className="text-xs font-bold text-blue-600 uppercase tracking-wider mb-3">관리자 답변</p>
